...
public interface RefreshTokenRepository extends JpaRepository<RefreshToken, Long> {
    ...
    @Query("SELECT r FROM RefreshToken r JOIN FETCH r.portalUser WHERE r.id=?1")
    @Override
    Optional<RefreshToken> findById(Long id);

    @Query("SELECT t FROM RefreshToken t" +
            " WHERE t.deactivatedAt IS NULL" +
            " AND t.actualAuthentication=?1" +
            " AND t.expiresAt>CURRENT_TIMESTAMP")
    List<RefreshToken> findActiveTokens(PortalUserAuthentication authenticationResponse);

    @Modifying
    @Query("UPDATE RefreshToken r SET r.deactivatedAt=CURRENT_TIMESTAMP" +
